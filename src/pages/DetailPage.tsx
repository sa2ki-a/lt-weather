...
export function DetailPage({location,onBack}:{location:Location;onBack:()=>void}){
  ...
  const dailyForecast=useWeather(location.latitude,location.longitude,'auto',mode==='daily')
  ...
  return <main className="detail">
    ...
    {mode==='daily'&&(dailyForecast.loading?<Loading/>:dailyForecast.error?<ErrorState message={dailyForecast.error} retry={dailyForecast.reload}/>:dailyForecast.data?<section><div className="timeline-heading"><div><h2>10日間タイムライン</h2><p>横にスクロールして日ごとに比較</p></div><span>今日から10日間</span></div><TenDayTimeline days={dailyForecast.data.daily} timezone={dailyForecast.data.timezone} latitude={location.latitude} longitude={location.longitude}/></section>:null)}

...
type HourlyApi={time:number[];temperature_2m:number[];relative_humidity_2m:number[];precipitation:number[];precipitation_probability:number[];wind_speed_10m:number[];cloud_cover:number[];weather_code:number[]}
...
export async function fetchWeather(latitude:number,longitude:number,modelId:WeatherModelId='auto',signal?:AbortSignal):Promise<WeatherData>{
  const hourlyFields='temperature_2m,relative_humidity_2m,precipitation,precipitation_probability,wind_speed_10m,cloud_cover,weather_code'
  ...
  return{timezone:data.timezone,timezoneAbbreviation:data.timezone_abbreviation,utcOffsetSeconds:data.utc_offset_seconds,hourly,daily,modelId,availableForecastDays}
}

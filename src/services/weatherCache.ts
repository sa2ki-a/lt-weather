...
const deserialize=(data:StoredWeatherData):WeatherData=>({...data,hourly:data.hourly.map(hour=>({...hour,time:new Date(hour.time)})),daily:data.daily.map(day=>({...day,date:new Date(day.date)}))})
...
export async function getWeather(latitude:number,longitude:number,kind:ForecastKind,model:WeatherModelId='auto',force=false):Promise<CachedWeatherResult>{const effectiveModel=kind==='daily'?'auto':model,key=cacheKey(latitude,longitude,kind,effectiveModel),cached=findEntry(key);if(!force&&cached&&Date.now()-cached.savedAt<=WEATHER_CACHE_TTL[kind])return{data:deserialize(cached.data),fetchedAt:cached.savedAt,stale:false};const pendingKey=requestKey(latitude,longitude,effectiveModel);let pending=inFlight.get(pendingKey);if(!pending){pending=fetchWeather(latitude,longitude,effectiveModel).then(data=>{const fetchedAt=Date.now();store(latitude,longitude,'hourly',effectiveModel,data,fetchedAt);if(effectiveModel==='auto')store(latitude,longitude,'daily','auto',data,fetchedAt);return{data,fetchedAt}}).finally(()=>inFlight.delete(pendingKey));inFlight.set(pendingKey,pending)}try{const result=await pending;return{...result,stale:false}}catch(error){if(cached)return{data:deserialize(cached.data),fetchedAt:cached.savedAt,stale:true};throw error}}

...
export interface HourlyWeather { time: Date; temperature: number; humidity: number; precipitation: number; precipitationProbability: number|null; windSpeed: number; windDirection: number; cloudCover: number; weatherCode: number }
...
export interface WeatherData { timezone: string; timezoneAbbreviation: string; utcOffsetSeconds: number; hourly: HourlyWeather[]; daily:DailyWeather[]; modelId:WeatherModelId; availableForecastDays:number; precipitationProbabilitySource:'model'|'best_match'|'unavailable' }

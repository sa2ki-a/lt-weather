...
export default defineConfig({
  plugins: [react(), VitePWA({
    // Let the app prompt before activating a newly downloaded worker.
    registerType: 'prompt',
    ...
    workbox: {
      ...
      runtimeCaching: [{ urlPattern: /^https:\/\/api\.open-meteo\.com\//, handler: 'NetworkFirst', options: { cacheName: 'weather-api', networkTimeoutSeconds: 8, expiration: { maxEntries: 30, maxAgeSeconds: 3600 } } }]
    }
  })]
})
